...
const compareBtn = document.getElementById('compareBtn'); // peut être null si pas d'id
...
const levels = {
  ...
  htmlcss: {
    easy: [
      { desc: 'Bouton stylisé', html: '<button>Cliquer</button>', css: 'button { background:#56D9BA; color:#161330; padding:12px 24px; border:none; border-radius:8px; font-weight:bold; cursor:pointer; }', hint: 'Stylez le bouton avec des couleurs et du padding' },
      { desc: 'Texte centré', html: '<div>Centré</div>', css: 'div { text-align:center; font-size:20px; color:#91E413; }', hint: 'Utilisez text-align: center' },
      ...
      { desc: 'Effet au survol', html: '<button>Survolez-moi</button>', css: 'button { background:#1CCAE8; color:white; padding:10px 20px; border:none; border-radius:5px; transition:0.3s; } button:hover { background:#91E413; transform:scale(1.05); }', hint: 'Ajoutez un état :hover avec transition' }
    ],
    medium: [
      { desc: 'Centrage avec Flexbox', html: '<div class="container"><span>Centré</span></div>', css: '.container { display:flex; align-items:center; justify-content:center; width:200px; height:150px; background:#161330; } span { color:#91E413; font-size:18px; }', hint: 'Utilisez les propriétés flexbox' },
      ...
      { desc: 'Bouton avec icône', html: '<button>⭐ Étoile</button>', css: 'button { background:linear-gradient(135deg, #667eea, #764ba2); color:white; border:none; padding:12px 20px; border-radius:8px; font-size:16px; cursor:pointer; box-shadow:0 4px 15px rgba(0,0,0,0.2); }', hint: 'Utilisez un dégradé en arrière-plan' }
    ],
    hard: [
...
function loadLevel() {
  ...
  // Placeholder éditeur
  if (currentMode === 'css') {
    editorEl.placeholder = 'Écrivez votre CSS ici...';
  } else if (currentMode === 'html') {
...
function handleCorrectAnswer() {
  ...
  // Autoriser la comparaison une fois le niveau réussi
  canCompare = true;
  ...
  // nombre de niveaux terminés = au moins currentLevelIndex + 1
  progress[currentMode][currentDifficulty] = Math.max(
    progress[currentMode][currentDifficulty],
    currentLevelIndex + 1
  );
  saveProgress(); // ne fait rien mais gardé pour structure
...
function skipLevel() {
  if (!gameStarted) {
    alert('Commence la partie avec "Démarrer" avant de passer un niveau.');
    return;
  }
